...
def iterate(positions, velocities):
    ...
    while True:
        ...
        for pos, vel in zip(positions, velocities):
            ...
            nPos.append(pos)
            nVel.append(vel)
        (positions, velocities) = (nPos, nVel)
...
def main():
    ...
    positions = list(map(parseLine, map(str.strip, open("input.txt").readlines())))
    velocities = [tuple(0 for _ in pos) for pos in positions]
    pprint(positions)
    pprint(velocities)

    simulation = enumerate(iterate(positions, velocities))
    simulation = islice(simulation, 1000, 1001)
    for i, (positions, velocities) in simulation:
        print_state(i, positions, velocities)
